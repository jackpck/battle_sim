...
class Battlefield:
    ...
    def update_state(self):
        for i in range(self.__regiment1.get_full_size()):
            self.state[i] = self.__regiment1.battalions[i].get_health()
        for i in range(self.__regiment2.get_full_size()):
    ...
    def commence_round(self):
        ...
        for bat in self.__regiment1.battalion_set:
            self.__regiment1.battalions[bat].count_KIA()
        for bat in self.__regiment2.battalion_set:
            self.__regiment2.battalions[bat].count_KIA()
    ...
    def get_reward(self):
        ...
        if result > 0:
            return 1 # regiment1 win
        elif result < 0:
            return -1 # regiment2 win
        else:
            return 0 # draw
